...
class NERModel(BaseModel):
    ...
    def add_word_embeddings_op(self):
        ...
        with tf.variable_scope("chars",reuse = tf.AUTO_REUSE):
            if self.config.use_chars:
                ...
                word_embeddings_dress_left_coup = tf.concat([word_embeddings_dress_left_coup, output], axis=-1) 
                ...
                word_embeddings_jean_left_coup = tf.concat([word_embeddings_jean_left_coup, output], axis=-1) 
                ...
                word_embeddings_dress_right_coup = tf.concat([word_embeddings_dress_right_coup, output], axis=-1)                    
                ...
                word_embeddings_jean_right_coup = tf.concat([word_embeddings_jean_right_coup, output], axis=-1)  
    ...
    def add_logits_op(self):
        ...
        with tf.variable_scope("bi-lstm", reuse=tf.AUTO_REUSE):
            stacked_cell_fw = []
            stacked_cell_bw = []
            layer_size = [self.config.hidden_size_lstm1,self.config.hidden_size_lstm2]
   
            for i in range(2):

                stacked_cell_fw.append(tf.contrib.rnn.LSTMCell(layer_size[i],name="fname" + str(i)))
                stacked_cell_bw.append(tf.contrib.rnn.LSTMCell(layer_size[i],name="bname" + str(i)))
        ...
        with tf.variable_scope("proj", reuse=tf.AUTO_REUSE):
            ...
            W = tf.get_variable("W", dtype=tf.float32,shape=[2*self.config.hidden_size_lstm2, self.config.ntags])
            b = tf.get_variable("b", shape=[self.config.ntags],
                    dtype=tf.float32, initializer=tf.zeros_initializer())

            nsteps = tf.shape(output)[1]
            output = tf.reshape(output, [-1, 2*self.config.hidden_size_lstm2])
    ...
    def add_coupling_loss_me(self):
        ...
        with tf.variable_scope("bi-lstm", reuse=tf.AUTO_REUSE):
            ...
            for i in range(2):
                ...
                stacked_cell_bw.append(tf.contrib.rnn.LSTMCell(layer_size[i],name="bname" + str(i)))           
            (output_fw, output_bw), _ = tf.nn.bidirectional_dynamic_rnn(
                    stacked_cell_fw[0], stacked_cell_bw[0], self.word_embeddings_dress_left_coup,
                    sequence_length=self.sequence_lengths_dress_left_coup, dtype=tf.float32)
            output = tf.concat([output_fw, output_bw], axis=-1)
            output = tf.nn.dropout(output, self.dropout)
            (output_fw1, output_bw1), _ = tf.nn.bidirectional_dynamic_rnn(
                    stacked_cell_fw[1], stacked_cell_bw[1], output, sequence_length=self.sequence_lengths_dress_left_coup ,dtype=tf.float32)
            output = tf.concat([output_fw1, output_bw1], axis=-1)
            output = tf.nn.dropout(output, self.dropout)
        ...
        with tf.variable_scope("bi-lstm", reuse=tf.AUTO_REUSE):
            ...
            for i in range(2):
                ...
                stacked_cell_bw.append(tf.contrib.rnn.LSTMCell(layer_size[i],name="bname" + str(i)))           
            (output_fw, output_bw), _ = tf.nn.bidirectional_dynamic_rnn(
                    stacked_cell_fw[0], stacked_cell_bw[0], self.word_embeddings_jean_left_coup,
                    sequence_length=self.sequence_lengths_jean_left_coup, dtype=tf.float32)
            output = tf.concat([output_fw, output_bw], axis=-1)
            output = tf.nn.dropout(output, self.dropout)
            (output_fw1, output_bw1), _ = tf.nn.bidirectional_dynamic_rnn(
                    stacked_cell_fw[1], stacked_cell_bw[1], output, sequence_length=self.sequence_lengths_jean_left_coup ,dtype=tf.float32)
            output = tf.concat([output_fw1, output_bw1], axis=-1)
            output = tf.nn.dropout(output, self.dropout)
        ...
        with tf.variable_scope("bi-lstm", reuse=tf.AUTO_REUSE):
            ...
            for i in range(2):
                ...
                stacked_cell_bw.append(tf.contrib.rnn.LSTMCell(layer_size[i],name="bname" + str(i)))           
            (output_fw, output_bw), _ = tf.nn.bidirectional_dynamic_rnn(
                    stacked_cell_fw[0], stacked_cell_bw[0], self.word_embeddings_dress_right_coup,
                    sequence_length=self.sequence_lengths_dress_right_coup, dtype=tf.float32)
            output = tf.concat([output_fw, output_bw], axis=-1)
            output = tf.nn.dropout(output, self.dropout)
            (output_fw1, output_bw1), _ = tf.nn.bidirectional_dynamic_rnn(
                    stacked_cell_fw[1], stacked_cell_bw[1], output, sequence_length=self.sequence_lengths_dress_right_coup ,dtype=tf.float32)
            output = tf.concat([output_fw1, output_bw1], axis=-1)
            output = tf.nn.dropout(output, self.dropout)
        ...
        with tf.variable_scope("bi-lstm", reuse=tf.AUTO_REUSE):
            ...
            for i in range(2):
                ...
                stacked_cell_bw.append(tf.contrib.rnn.LSTMCell(layer_size[i],name="bname" + str(i)))           
            (output_fw, output_bw), _ = tf.nn.bidirectional_dynamic_rnn(
                    stacked_cell_fw[0], stacked_cell_bw[0], self.word_embeddings_jean_right_coup,
                    sequence_length=self.sequence_lengths_jean_right_coup, dtype=tf.float32)
            output = tf.concat([output_fw, output_bw], axis=-1)
            output = tf.nn.dropout(output, self.dropout)
            (output_fw1, output_bw1), _ = tf.nn.bidirectional_dynamic_rnn(
                    stacked_cell_fw[1], stacked_cell_bw[1], output, sequence_length=self.sequence_lengths_jean_right_coup ,dtype=tf.float32)
            output = tf.concat([output_fw1, output_bw1], axis=-1)
            output = tf.nn.dropout(output, self.dropout)
    ...
    def predict_batch1(self, words):
        ...
        for i in tg:
            tgs.append(i.strip())
        tagsv1 = []
        for idx , i in enumerate(tgs):
            if("_dress" in i or i=='O'):
                tagsv1.append(idx)
        ...
        if self.config.use_crf:
            ...
            if perform_op_coup:
                ...
                logits, trans_params = self.sess.run(
                     [self.logits, self.trans_params], feed_dict=fd)
            trans_params = trans_params[tagsv1,:][:,tagsv1]
            #print("todo line 624 @ner_model")
            # iterate over the sentences because no batching in vitervi_decode
            for logit, sequence_length in zip(logits, sequence_lengths):
                logit = logit[:sequence_length] # keep only the valid steps
                logit = logit[:,tagsv1]
                viterbi_seq_temp, viterbi_score = tf.contrib.crf.viterbi_decode(
                        logit, trans_params)
                viterbi_seq = []
                for sq in viterbi_seq_temp:
                    viterbi_seq.append(tagsv1[sq])
                viterbi_sequences += [viterbi_seq]
    ...
    def predict_batch2(self, words):
        ...
        if self.config.use_crf:
            ...
            if perform_op_coup:
                ...
                logits, trans_params = self.sess.run(
                     [self.logits, self.trans_params], feed_dict=fd)
            trans_params = trans_params[tagsv2,:][:,tagsv2]
            #print("todo line 624 @ner_model")
            # iterate over the sentences because no batching in vitervi_decode
            for logit, sequence_length in zip(logits, sequence_lengths):
                logit = logit[:sequence_length] # keep only the valid steps
    ...
    def run_epoch(self, train, dev, epoch,data_for_copuling):
        ...
        metrics = self.run_evaluate(dev,1)
    ...
    def run_evaluate2(self, test, k ):
        ...
        correct_preds, total_correct, total_preds = 0., 0., 0.
        for words, labels, sumit in minibatches(test, self.config.batch_size,None,None):
            labels_pred, sequence_lengths = self.predict_batch2(words)
            for desc, lab, lab_pred, length in zip(sumit, labels, labels_pred,
                                             sequence_lengths):
