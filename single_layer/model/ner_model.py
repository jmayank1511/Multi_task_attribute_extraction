...
class NERModel(BaseModel):
    ...
    def add_logits_op(self):
        ...
        with tf.variable_scope("bi-lstm", reuse=tf.AUTO_REUSE):
            stacked_cell_fw = []
            stacked_cell_bw = []
            layer_size = [self.config.hidden_size_lstm1]
   


            stacked_cell_fw.append(tf.contrib.rnn.LSTMCell(layer_size[0],name="fname" ))
            stacked_cell_bw.append(tf.contrib.rnn.LSTMCell(layer_size[0],name="bname" ))
        ...
        with tf.variable_scope("proj", reuse=tf.AUTO_REUSE):
            ...
            W = tf.get_variable("W", dtype=tf.float32,shape=[2*self.config.hidden_size_lstm1, self.config.ntags])
            b = tf.get_variable("b", shape=[self.config.ntags],
                    dtype=tf.float32, initializer=tf.zeros_initializer())

            nsteps = tf.shape(output)[1]
            output = tf.reshape(output, [-1, 2*self.config.hidden_size_lstm1])
    ...
    def add_coupling_loss_me(self):
        ...
        with tf.variable_scope("proj", reuse=tf.AUTO_REUSE):          
            W = tf.get_variable("W", dtype=tf.float32,
                    shape=[2*self.config.hidden_size_lstm1, self.config.ntags])
            b = tf.get_variable("b", shape=[self.config.ntags],
                    dtype=tf.float32, initializer=tf.zeros_initializer())
            nsteps = tf.shape(output)[1]
            output = tf.reshape(output, [-1, 2*self.config.hidden_size_lstm1])
